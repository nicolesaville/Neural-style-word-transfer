...
def build_parser():
    ...
    parser.add_argument('--out-path', type=str,
                        dest='out_path', help='dir or file of transformed file or files', metavar='OUT_PATH',
                        required=True)
            ...
            if is_paths:
                ...
                for j, path_in in enumerate(curr_batch_in):
                    ...
                    assert img.shape == img_shape, \
                        'Images have different dimensions. Resize images or use --allow-different-dimensions.'
                    X[j] = img
            else:
...
# Perform feed-forward transfer on single input image
def ffwd_to_img(in_path, out_path, checkpoint_dir, device='/cpu:0'):
    paths_in, paths_out = [in_path], [out_path]
    ffwd(paths_in, paths_out, checkpoint_dir, batch_size=1, device_t=device)


# Perform feed-forward transfer on images of different dimensions
