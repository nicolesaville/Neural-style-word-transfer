...
def conditional_instance_norm(inputs,
  ...
  with tf.variable_scope(scope, 'InstanceNorm', [inputs],
    ...
    if center:
      beta = _label_conditioned_variable('beta', tf.zeros_initializer(), labels, num_categories)
    if scale:
      gamma = _label_conditioned_variable('gamma', tf.ones_initializer(), labels, num_categories)
    ...
    #print("\n inputs:", inputs, "input shape:", tf.shape(inputs), "outputs:", outputs, "output shape:", tf.shape(outputs), "\n")
    outputs.set_shape(inputs_shape)
    if activation_fn:
      outputs = activation_fn(outputs)
    return slim.utils.collect_named_outputs(outputs_collections, sc.original_name_scope, outputs)
